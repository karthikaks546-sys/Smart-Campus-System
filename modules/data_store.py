...
USER_FIELDS     = ["student_id", "name", "age", "email", "contact", "password_hash"]
...
def get_student(student_id: str) -> dict | None:
    return next((s for s in get_students() if s["student_id"] == student_id), None)
...
def add_user(user: dict):
    append_csv(USERS_FILE, user, USER_FIELDS)


def update_user(student_id: str, updated: dict):
    rows = get_users()
    rows = [updated if r["student_id"] == student_id else r for r in rows]
    write_csv(USERS_FILE, rows, USER_FIELDS)


def delete_user(student_id: str):
    rows = [r for r in get_users() if r["student_id"] != student_id]
    write_csv(USERS_FILE, rows, USER_FIELDS)


def user_exists(student_id: str) -> bool:
    return any(u["student_id"] == student_id for u in get_users())


def get_user(student_id: str) -> dict | None:
    return next((u for u in get_users() if u["student_id"] == student_id), None)


def get_user_by_email(email: str) -> dict | None:
    return next((u for u in get_users() if u.get("email", "").strip().lower() == email.strip().lower()), None)


def authenticate_user(student_id: str, password: str, verify_password_fn) -> bool:
    user = get_user(student_id)
    if not user:
        return False
    return verify_password_fn(password, user.get("password_hash", ""))
